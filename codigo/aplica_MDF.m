function [A, iter] = ...
    ...
    elseif item == "e2"
        ...
        % Equações de pontos fora da bobina
        Aij_int_fora = equacoes{2};
        Aij_vert_fora = equacoes{3};
        Aij_hori_fora = equacoes{4};
        ...
        for t=Tempo
            ...
            for j=2:rows-1
                for i=2:col_eq(20)
                    ...
                    % Escolhe qual equação de Aij utilizar
                    if Fronteiras(j, i) == vertical % se é fronteira vertical
                        A(j, i, k+1) = Aij_vert_fora(A, j, i, k, t,...
                        MIx(j,i-1), MIy(j,i-1), JZ(j,i-1), ...
                        MIx(j,i+1), MIy(j,i+1), JZ(j,i+1));
                    elseif Fronteiras(j, i) == horizontal % se é fronteira horizontal
                        A(j, i, k+1) = Aij_hori_fora(A, j, i, k, t,...
                        MIx(j-1,i), MIy(j-1,i), JZ(j-1,i), ...
                        MIx(j+1,i), MIy(j+1,i), JZ(j+1,i));
                    else % se estiver em um ponto interior do domínio
                        A(j, i, k+1) = Aij_int_fora(A, j, i, k, t,...
                        MIx(j, i), MIy(j, i), JZ(j, i));
                    end                                   
