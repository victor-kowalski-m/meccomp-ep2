...
Aij_2mis_interior = @(A, j, i, mix, miy, Jz)...
    ...
    (A(k,(-1)+j,i)+(-2).*A(k,j,i)+A(k,1+j,i))+mix.* ...
    (dt.*(A(k,j,(-1)+i)+(-2).*A(k,j,i)+A(k,j,1+i))+dx2.*miy.* ...
...
Aij_2mis_front_vert = @(A, j, i, mix1, miy1, Jz1, mix2, miy2, Jz2)...
    ...
    A(k,j,i)+dt.*(2.*mix1.*mix2.*miy1.*A(k,j,1+i)+miy2.* ...
    (2.*mix1.*mix2.*A(k,j,(-1)+i)+miy1.*((mix1+mix2).* ...
    A(k,(-1)+j,i)+(mix1+mix2).*A(k,1+j,i)+dx2.* ...
