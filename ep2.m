...
% Função de cálculo de Aijk em um ponto no interior do domínio
Aij2mis_interior = @(A, j, i, mi, Jz)...
    ((1/2).*(mix+miy).^(-1).*(dx2.*Jz.*mix.*miy+mix.*...
    (A(k,j,i-1)+A(k,j,i+1))+miy.*(A(k,j-1,i)+A(k,j+1,i))));

% Função de cálculo de Aijk na fronteira vertical entre dois meios
Aij2mis_front_vert = @(A, j, i, mi1, Jz1, mi2, Jz2)...
    ((1/2).*(mix2.*miy1.*miy2+mix1.*(miy1.*miy2+mix2.*...
    (miy1+miy2))).^(-1).*(2.*mix1.*mix2.*miy1.*A(k,j,1+i)+miy2.*...
    (2.*mix1.*mix2.*A(k,j,(-1)+i)+miy1.*(dx2.*(Jz1+Jz2).*...
    mix1.*mix2+(mix1+mix2).*A(k,(-1)+j,i)+(mix1+mix2).*A(k,1+j,i)))));

% Função de cálculo de Aijk na fronteira horizontal entre dois meios
Aij2mis_front_hori = @(A, j, i, mi1, Jz1, mi2, Jz2) ...
    ((1/2).*(mix2.*miy1.*miy2+mix1.*(miy1.*miy2+mix2.*...
    (miy1+miy2))).^(-1).*(mix1.*mix2.*(miy1+miy2).*A(k,j,(-1)+i)+...
    mix1.*mix2.*(miy1+miy2).*A(k,j,1+i)+miy1.*miy2.*...
    (mix2.*(dx2.*(Jz1+Jz2).*mix1+2.*A(k,(-1)+j,i))+2.*mix1.*A(k,1+j,i))));
